class Servicedependency
	@@o = %w[dependent_host_name dependent_hostgroup_name dependent_service_description]
	@@o += %w[host_name hostgroup_name register name]
	@@o += %w[service_description inherits_parent dependency_period]
	@@o += %w[execution_failure_criteria notification_failure_criteria]
	@@o.each {|x| send(:attr_accessor, x)}
	@@servicedepencys = Hash.new
	...
	def _register
		...
		when nil
			...
			when nil
				cn = "#{self.dependent_hostgroup_name}|#{self.dependent_service_description}"
			else
				cn = "#{self.dependent_host_name}|#{self.dependent_service_description}"
			end
		end
